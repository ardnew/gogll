...
func getCondition(event ast.LexBase) string {
	...
	case *ast.UnicodeSet:
		return getUnicodeSetCondition(e)
	}
	panic(fmt.Sprintf("Invalid event %T", event))
}

func getUnicodeSetCondition(e *ast.UnicodeSet) string {
	incl, excl := getInclExclClasses(e)
	w := new(bytes.Buffer)
	fmt.Fprintf(w, "unicode.In(r")
	for _, class := range incl {
		fmt.Fprintf(w, ", _%s", class)
	}
	fmt.Fprint(w, ")")
	if len(excl) > 0 {
		fmt.Fprint(w, " && !unicode.In(r")
		for _, class := range excl {
			fmt.Fprintf(w, ", _%s", class)
		}
		fmt.Fprint(w, ")")
	}
...
func getInclExclClasses(e *ast.UnicodeSet) (incl, excl []string) {
	for _, rng := range e.Ranges {
		if rng.Exclude {
			excl = append(excl, rng.Range)
		} else {
			incl = append(incl, rng.String())
		}
	}
	return
}
